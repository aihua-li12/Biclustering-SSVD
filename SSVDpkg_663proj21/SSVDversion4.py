## This is a modified version of SSVDversion3. 
## The broadcasting may result in extremely huge matrix (as we have to compute the outer product),
## which can be counterproductive if it indeed slows the computation. 
## In this version, we appropriately combine broadcasting and the map loop. 

import numpy as np 
import scipy.linalg as la


def get_outer_prod(tilde_vec, fixed_vec, fixed_name):
    """Compute the outer product according to the formula. """
    if fixed_name == "u":
        return np.outer(fixed_vec, tilde_vec)
    else:
        return np.outer(tilde_vec, fixed_vec)
...
def update_uv(u_old, v_old, X, gamma1, gamma2, lam_grid):
    ...
    SSEs_v = list(map(lambda tilde_vec: np.sum((X - get_outer_prod(tilde_vec, u_old, "u"))**2), v_tilde_br.T))  # (S,)
    ...
    if not np.all(part2 == 0):  # not full shrinkage at v
        ...
        SSEs_u = list(map(lambda tilde_vec: np.sum((X - get_outer_prod(tilde_vec, v_new, "v"))**2), u_tilde_br.T))  # (S,)
