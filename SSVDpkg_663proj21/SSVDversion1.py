## This the most basic realization of the SSVD algorithm.
## Codes directly correspond to the paper context. 
...
def get_BIC(lam, w, tilde_hat, fixed_vec, response, sigma2_hat, nd):
    ...
    tilde = get_tilde(lam, tilde_hat, w)  # get the tilde vector udner the current lambda
    response_hat = np.kron(np.eye(tilde.shape[0]), fixed_vec) @ tilde
    BIC = np.linalg.norm(response-response_hat)**2/(nd*sigma2_hat)+np.log(nd)/nd*df
    return BIC
def select_lam(lam_grid, w, tilde_hat, fixed_vec, response, sigma2_hat, nd):
    ...
    """
    BICs = list(map(lambda lam: get_BIC(lam, w, tilde_hat, fixed_vec, response, sigma2_hat, nd), lam_grid))
    return lam_grid[np.argmin(BICs)]
def update_uv(u_old, v_old, X, Y, Z, gamma1, gamma2, lam_grid):
    ...
    Y_hat = (np.kron(np.eye(d), u_old) @ v_tilde_hat).reshape((-1,1))  # (nd, 1), ols estimate
    sigma2_hat_v = ((Y-Y_hat).T @ (Y-Y_hat) / (nd-d))[0][0]  # scalar, fixed ols estimate

    # select lambda_v
    w2 = np.abs(v_tilde_hat)**(-gamma2)  # (d, 1)
    lambda_v = select_lam(lam_grid, w2, v_tilde_hat, u_old, Y, sigma2_hat_v, nd)
    ...
    if np.all(v_tilde == 0):  # full shrinkage at v
        ...
        Z_hat = np.kron(np.eye(n), v_new) @ u_tilde_hat  # (nd, 1), ols estimate
        sigma2_hat_u = ((Z-Z_hat).T @ (Z-Z_hat) / (nd-n))[0][0]  # scalar, fixed ols estimate

        # select lambda_u
        w1 = np.abs(u_tilde_hat)**(-gamma1)
        lambda_u = select_lam(lam_grid, w1, u_tilde_hat, v_new, Z, sigma2_hat_u, nd)
...
def SSVD_layer(X, lam_grid, gamma1, gamma2, max_iter=5000, tol=1e-6):
    ...
    for i in range(max_iter):
        u_new, v_new, lambda_u, lambda_v = update_uv(u_old, v_old, X, Y, Z, gamma1, gamma2, lam_grid)
